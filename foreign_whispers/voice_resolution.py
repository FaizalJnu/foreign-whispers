...
def resolve_speaker_wav(
    ...
    if speaker_id:
        speaker_wav = f"{speaker_id}.wav"
        if (speakers_dir / target_language / speaker_wav).exists():
            return f"{target_language}/{speaker_wav}"

    # 2. Language-specific default
    if (speakers_dir / target_language / "default.wav").exists():
